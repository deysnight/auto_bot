...
class Storage {
  constructor() {
    _defineProperty(this, "ff_checkTimeout", (currency, timeout) => {
      var now = Date.now();
      var last = this.ff_data[currency].lastDone;
    ...
    _defineProperty(this, "ff_update_last", currency => {
      this.ff_data[currency].lastDone = Date.now();
    });

    _defineProperty(this, "fc_checkTimeout", timeout => {
      var now = Date.now();
      Math.min(2, 3, 1);
      var last = Math.min(this.fc_data.ptc.lastDone, this.fc_data.shortlink.lastDone, this.fc_data.faucet.lastDone);
    ...
    _defineProperty(this, "fc_update_last", side => {
      if (side === "ptc") {
